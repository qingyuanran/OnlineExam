...
public class TktInterfaceImplDao implements TktInterfaceDao {
	...
	public boolean insert(Tkt tkt) {
		...
		String sql = "insert into tkt(question,answer,questiontype,questionpoint) values (?,?,?,?)";
		try {
			pr = con.prepareStatement(sql);
			System.out.println("！！！！"+tkt.getQuestion());
			System.out.println("！！！！"+tkt.getAnswer());
			pr.setString(1, tkt.getQuestion());
			pr.setString(2, tkt.getAnswer());
			pr.setInt(3, 2);
			pr.setString(4, tkt.getQuestionpoint());
	...
	public int selectId(String question) {
		int id = 0;
		String sql="select * from tkt where question=?";
		con=DBUtil.getConnection();
		try {
			pr=con.prepareStatement(sql);
